...
# --- Widget to preview the chord diagram ---
class ChordPreviewWidget(QWidget):
    ...
    def paintEvent(self, event):
        ...
        strings = self.parent_tab.strings
        frets = len(self.parent_tab.buttons[0])
        # vertical
        for i in range(strings):
            x = margin + i * (width/(strings-1))
            painter.drawLine(x, margin, x, margin+height)
        # horizontal frets
        for j in range(frets):
            y = margin + j * (height/(frets-1))
            painter.drawLine(margin, y, margin+width, y)
        # draw markers
        for i in range(strings):
            for j, btn in enumerate(self.parent_tab.buttons[i]):
                state = btn.get_state()
                if state != 'none':
                    x = margin + i * (width/(strings-1))
                    y = margin + j * (height/(frets-1))
...
class ChordTab(QWidget):
    ...
    def init_ui(self):
        ...
        # Preview widget
        self.preview = ChordPreviewWidget(self)
